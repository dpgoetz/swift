...
class TestStaticLargeObject(unittest.TestCase):
    ...
    def test_handle_multipart_get_bad_request(self):
        req = Request.blank(
        ...
        self.app.calls = 0
        req = Request.blank(
            '/test_get_not_slo/A/c/man?multipart-manifest=get',
            environ={'REQUEST_METHOD': 'GET'})
        resp = self.slo.handle_multipart_get(req)
        self.assertEquals(self.app.calls, 1)
        self.assertEquals(resp.status_int, 400)
    ...
    def test_handle_multipart_head(self):
        good_data = json.dumps(
            [{'path': '/c/a_1', 'etag': 'a', 'size_bytes': '1'},
             {'path': '/d/b_2', 'etag': 'b', 'size_bytes': '2'}])
        req = Request.blank(
        ...
        self.assertEquals(resp.headers['x-static-large-object'], 'True')
        self.assertEquals(resp.headers['Content-Type'], 'html')
        self.assertEquals(resp.content_length, len(good_data))
