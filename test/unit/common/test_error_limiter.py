...
class TestErrorLimiter(unittest.TestCase):
    def setUp(self):
        self.limiter = ErrorLimiter('default')
        self.limiter.db_dir = tempfile.mkdtemp()
    ...
    def my_get_db_path(self, offset=0):
        ...
        return os.path.join(
            self.limiter.db_dir,
            'error_limiter.%d.db' % (
                int(self.start_time / self.limiter.db_lifetime) - total_offset))
    ...
    def test_populate_from_old_db_with_min_time(self):
        node = {'ip': '1.2.3.4', 'port': 6000}
        other_limiter = ErrorLimiter('default')
    ...
    def test_get_current_state_exception_handling(self):
        ...
        with patch.object(self.limiter, 'process_error_limit_rows', my_func):
            self.assertRaises(sqlite3.OperationalError, self.limiter.get_current_state)
