        ...
        try:
            ...
            if self.is_slo:
                if (resp.etag != self.segment_dict['hash'] or
                    ...
                    # possible that a attacker could upload a >1mb object and
                    # then replace with a much smaller object with same etag.
                    # Then create a big nested SLO that calls that object many
                    # times which would hammer our obj servers. If this is a
                    # range request, don't check content-length because it
                    # won't match.
                    raise SegmentError(_(
...
class ObjectController(Controller):
    ...
    def _slo_listing_obj_iter(self, incoming_req, account, container, obj,
        ...
        create an iterable that will expand nested SLOs at it walk though the
        listing.
