...
from swift.common.utils import renamer, lock_parent_directory, \
    get_logger, remove_file, readconf
...
class ErrorLimiter(object):
    ...
    def __init__(self, conf_path, logger=None):
        ...
        self.window = int(conf.get('window_size', 60))
        self.timeout = float(conf.get('db_timeout', 1))
    ...
    def populate_from_old_db(self, cur_conn):
        ...
        with self.get_conn() as conn:
            ...
            min_previous_time = int(
                time() - (self.window * self.lookback_windows))
            try:
                ...
                FROM previous.error_limiter p WHERE p.time_window < (
                SELECT MIN(time_window) FROM error_limiter)
                AND p.time_window > ?
                ''', (min_previous_time,))
                conn.commit()
            except sqlite3.OperationalError, e:
    ...
    def process_error_limit_rows(self, curs):
        ...
        for row in curs:
            ...
            numerator = node_to_dict.get(row_node_id, {}).get('numerator', 0.0)
            denominator = \
                node_to_dict.get(row_node_id, {}).get('denominator', 0.0)
            num_results = node_to_dict.get(
                row_node_id, {}).get('num_results', self.lookback_windows)
            was_num_reqs = node_to_dict.get(row_node_id, {}).get('num_reqs', 0)
            was_num_errors = \
                node_to_dict.get(row_node_id, {}).get('num_errors', 0)

            if num_results <= 0:
                continue
            row_average = float(num_errors) / num_reqs
            numerator += row_average * num_results
            denominator += num_results
            num_results -= 1
            ...
            node_to_dict[row_node_id] = {
                ...
                'num_reqs': was_num_reqs + num_reqs,
                'num_errors': was_num_errors + num_errors}
        return node_to_dict

    def get_min_time(self):
        time_window = self.get_window()
        return time_window - self.window * self.lookback_windows
    ...
    def is_error_limited(self, node):
        ...
        with self.get_conn() as conn:
            ...
            return random.random() < weighted_average
